...
*/
static int SDL_imageFilterAddMMX(
    unsigned char *Src1,
    unsigned char *Src2,
    unsigned char *Dest,
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterAdd(unsigned char *Src1, unsigned char *Src2, unsigned char *Dest, const unsigned int length) {
    unsigned int istart;
    ...
    for (unsigned int i = istart; i < length; i++) {
        int result = (int)*cursrc1 + (int)*cursrc2;
        if (result > 255)
            result = 255;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterMeanMMX(
    unsigned char *Src1,
    unsigned char *Src2,
    unsigned char *Dest,
    unsigned int SrcLength,
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterMean(unsigned char *Src1, unsigned char *Src2, unsigned char *Dest, const unsigned int length) {
    ...
    /* C routine to process image */
    for (unsigned int i = istart; i < length; i++) {
	    const int result = (int)*cursrc1 / 2 + (int)*cursrc2 / 2;
        *curdst = (unsigned char)result;
        /* Advance pointers */
        cursrc1++;
        cursrc2++;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterSubMMX(
    unsigned char *Src1,
    unsigned char *Src2,
    unsigned char *Dest,
    unsigned int SrcLength
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterSub(unsigned char *Src1, unsigned char *Src2, unsigned char *Dest, const unsigned int length) {
    unsigned int istart;
    unsigned char *cursrc1, *cursrc2, *curdst;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterAbsDiffMMX(
    unsigned char *Src1,
    unsigned char *Src2,
    unsigned char *Dest,
    unsigned int SrcLength
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterAbsDiff(unsigned char *Src1, unsigned char *Src2, unsigned char *Dest, const unsigned int length) {
    unsigned int istart;
    unsigned char *cursrc1, *cursrc2, *curdst;
    ...
    /* C routine to process image */
    for (unsigned int i = istart; i < length; i++) {
	    const int result = abs((int)*cursrc1 - (int)*cursrc2);
        *curdst = (unsigned char)result;
        /* Advance pointers */
        cursrc1++;
        cursrc2++;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterMultMMX(
    unsigned char *Src1,
    unsigned char *Src2,
    unsigned char *Dest,
    unsigned int SrcLength
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterMult(unsigned char *Src1, unsigned char *Src2, unsigned char *Dest, const unsigned int length) {
    unsigned int istart;
    unsigned char *cursrc1, *cursrc2, *curdst;
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterMultNor(unsigned char *Src1, unsigned char *Src2, unsigned char *Dest, const unsigned int length) {
    unsigned int istart;
    unsigned char *cursrc1, *cursrc2, *curdst;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterMultDivby2MMX(
    unsigned char *Src1,
    unsigned char *Src2,
    unsigned char *Dest,
    unsigned int SrcLength
...
int SDL_imageFilterMultDivby2(
    ...
    /* C routine to process image */
    for (unsigned int i = istart; i < length; i++) {
        int result = (int)*cursrc1 / 2 * (int)*cursrc2;
        if (result > 255)
            result = 255;
        *curdst = (unsigned char)result;
        /* Advance pointers */
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterMultDivby4MMX(
    unsigned char *Src1,
    unsigned char *Src2,
    unsigned char *Dest,
    unsigned int SrcLength
...
int SDL_imageFilterMultDivby4(
    ...
    /* C routine to process image */
    for (unsigned int i = istart; i < length; i++) {
        int result = (int)*cursrc1 / 2 * ((int)*cursrc2 / 2);
        if (result > 255)
            result = 255;
        *curdst = (unsigned char)result;
        /* Advance pointers */
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterBitAndMMX(
    unsigned char *Src1,
    unsigned char *Src2,
    unsigned char *Dest,
    unsigned int SrcLength
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterBitAnd(unsigned char *Src1, unsigned char *Src2, unsigned char *Dest, const unsigned int length) {
    unsigned int istart;
    unsigned char *cursrc1, *cursrc2, *curdst;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterBitOrMMX(
    unsigned char *Src1,
    unsigned char *Src2,
    unsigned char *Dest,
    unsigned int SrcLength
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterBitOr(unsigned char *Src1, unsigned char *Src2, unsigned char *Dest, const unsigned int length) {
    unsigned int istart;
    unsigned char *cursrc1, *cursrc2, *curdst;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterDivASM(
    unsigned char *Src1,
    unsigned char *Src2,
    unsigned char *Dest,
    unsigned int SrcLength
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterDiv(unsigned char *Src1, unsigned char *Src2, unsigned char *Dest, const unsigned int length) {
    /* Validate input parameters */
    if (Src1 == NULL || Src2 == NULL || Dest == NULL)
        return -1;
    if (length == 0)
    ...
    const unsigned char *cursrc1 = Src1;
    const unsigned char *cursrc2 = Src2;
    unsigned char *curdst = Dest;
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterBitNegation(unsigned char *Src1, unsigned char *Dest, const unsigned int length) {
    unsigned int istart;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterAddByteMMX(
    unsigned char *Src1,
    unsigned char *Dest,
    unsigned int SrcLength,
    unsigned char C
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterAddByte(unsigned char *Src1, unsigned char *Dest, const unsigned int length, const unsigned char C) {
    unsigned int istart;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterAddUintMMX(
    unsigned char *Src1,
    unsigned char *Dest,
    unsigned int SrcLength,
    unsigned int C,
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterAddUint(unsigned char *Src1, unsigned char *Dest, const unsigned int length, const unsigned int C) {
    unsigned int istart;
    int iC[4];
    ...
    if (SDL_imageFilterMMXdetect() && length > 7) {
        /* MMX routine */
        const unsigned int D = SWAP_32(C);
    ...
    iC[1] = (int)(C >> 8 & 0xff);
    iC[0] = (int)(C >> 0 & 0xff);
    for (unsigned int i = istart; i < length; i += 4) {
        for (unsigned int j = 0; j < 4; j++) {
            if (i + j < length) {
                int result = (int)*cursrc1 + iC[j];
                if (result > 255)
                    result = 255;
                *curdest = (unsigned char)result;
                /* Advance pointers */
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterAddByteToHalfMMX(
    unsigned char *Src1,
    unsigned char *Dest,
    unsigned int SrcLength,
    unsigned char C,
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterSubByte(unsigned char *Src1, unsigned char *Dest, const unsigned int length, const unsigned char C) {
    unsigned int istart;
    unsigned char *cursrc1;
    unsigned char *curdest;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterSubUintMMX(
    unsigned char *Src1,
    unsigned char *Dest,
    unsigned int SrcLength,
    unsigned int C,
...
\return Returns 0 for success or -1 for error.
*/
int SDL_imageFilterSubUint(unsigned char *Src1, unsigned char *Dest, const unsigned int length, const unsigned int C) {
    unsigned int istart;
    int iC[4];
    unsigned char *cursrc1;
    unsigned char *curdest;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterShiftRightMMX(
    unsigned char *Src1,
    unsigned char *Dest,
    unsigned int SrcLength,
    unsigned char N,
...
int SDL_imageFilterShiftRightUint(
    ...
    /* C routine to process image */
    const unsigned int *icursrc1 = (unsigned int *)cursrc1;
    unsigned int *icurdest = (unsigned int *)curdest;
    for (unsigned int i = istart; i < length; i += 4) {
        if (i + 4 < length) {
	        const unsigned int result = (unsigned int)*icursrc1 >> N;
            *icurdest = result;
        }
        /* Advance pointers */
        icursrc1++;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterMultByByteMMX(
    unsigned char *Src1,
    unsigned char *Dest,
    unsigned int SrcLength,
    unsigned char C
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterShiftRightAndMultByByteMMX(
    unsigned char *Src1,
    unsigned char *Dest,
    unsigned int SrcLength,
    unsigned char N,
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterShiftLeftByteMMX(
    unsigned char *Src1,
    unsigned char *Dest,
    unsigned int SrcLength,
    unsigned char N,
...
int SDL_imageFilterShiftLeftByte(
    ...
    /* C routine to process image */
    for (unsigned int i = istart; i < length; i++) {
	    const int result = (int)*cursrc1 << N & 0xff;
        *curdest = (unsigned char)result;
        /* Advance pointers */
        cursrc1++;
        curdest++;
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterBinarizeUsingThresholdMMX(
    unsigned char *Src1,
    unsigned char *Dest,
    unsigned int SrcLength,
    unsigned char T
...
\return Returns 0 for success or -1 for error.
*/
static int SDL_imageFilterClipToRangeMMX(
    unsigned char *Src1,
    unsigned char *Dest,
    unsigned int SrcLength,
    unsigned char Tmin,
...
int SDL_imageFilterNormalizeLinear(
    ...
    if (dC == 0)
        return 0;
    const int dN = Nmax - Nmin;
    const int factor = dN / dC;
    for (unsigned int i = istart; i < length; i++) {
        int result = factor * ((int)*cursrc - Cmin) + Nmin;
        if (result > 255)
            result = 255;
        *curdest = (unsigned char)result;
        /* Advance pointers */
...
\return Returns 1 if filter was applied, 0 otherwise.
*/
int SDL_imageFilterConvolveKernel9x9Divide(
    const unsigned char *Src,
    const unsigned char *Dest,
    int rows,
    int columns,
...
\return Returns 1 if filter was applied, 0 otherwise.
*/
int SDL_imageFilterConvolveKernel3x3ShiftRight(
    const unsigned char *Src,
    const unsigned char *Dest,
    int rows,
    int columns,
...
\return Returns 1 if filter was applied, 0 otherwise.
*/
int SDL_imageFilterConvolveKernel5x5ShiftRight(
    const unsigned char *Src,
    const unsigned char *Dest,
    int rows,
    int columns,
...
\return Returns 1 if filter was applied, 0 otherwise.
*/
int SDL_imageFilterConvolveKernel7x7ShiftRight(
    const unsigned char *Src,
    const unsigned char *Dest,
    int rows,
    int columns,
...
\return Returns 1 if filter was applied, 0 otherwise.
*/
int SDL_imageFilterSobelX(const unsigned char *Src, const unsigned char *Dest, int rows, int columns) {
    /* Validate input parameters */
    if (Src == NULL || Dest == NULL)
        return -1;
...
\return Returns 1 if filter was applied, 0 otherwise.
*/
int SDL_imageFilterSobelXShiftRight(
    const unsigned char *Src,
    const unsigned char *Dest,
    int rows,
    int columns,
