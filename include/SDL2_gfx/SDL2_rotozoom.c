...
*/
int shrinkSurfaceRGBA(const SDL_Surface *src, const SDL_Surface *dst, const int factorx, const int factory) {
    int ga, ba, aa;
    ...
    */
    ...
    const int n_average = factorx * factory;

    /*
    * Scan destination
    */
    tColorRGBA *sp = src->pixels;

    tColorRGBA *dp = dst->pixels;
    const int dgap = dst->pitch - dst->w * 4;
...
\return 0 for success or -1 for error.
*/
int shrinkSurfaceY(const SDL_Surface *src, const SDL_Surface *dst, const int factorx, const int factory) {
    /*
    * Averaging integer shrink
    */
    ...
    */
    ...
    Uint8 *dp = dst->pixels;
    const int dgap = dst->pitch - dst->w;
...
\return 0 for success or -1 for error.
*/
int zoomSurfaceRGBA(SDL_Surface *src, SDL_Surface *dst, const int flipx, const int flipy, const int smooth) {
...
\return 0 for success or -1 for error.
*/
int zoomSurfaceY(const SDL_Surface *src, const SDL_Surface *dst, const int flipx, const int flipy) {
    int x, y;
    Uint32 *sax, *say;
...
void transformSurfaceY(
    ...
    * Variable setup
    */
    const int xd = src->w - dst->w << 15;
    const int yd = src->h - dst->h << 15;
    const int ax = (cx << 16) - icos * cx;
    const int ay = (cy << 16) - isin * cx;
    tColorY *pc = dst->pixels;
    const int gap = dst->pitch - dst->w;
    /*
    * Clear surface to colorkey
    */
    memset(pc, colorkey(src) & 0xff, dst->pitch * dst->h);
    /*
...
*/
SDL_Surface *rotateSurface90Degrees(SDL_Surface *src, const int numClockwiseTurns) {
    int row, col, newWidth, newHeight;
    Uint8 *srcBuf;
    Uint8 *dstBuf;

    /* Has to be a valid surface pointer and be a Nbit surface where n is divisible by 8 */
    if (src == NULL || src->format == NULL) {
    ...
    int normalizedClockwiseTurns = (numClockwiseTurns % 4);
    ...
    const int bpp = src->format->BitsPerPixel / 8;
...
void rotozoomSurfaceSizeTrig(
    ...
    *canglezoom *= zoomy;
    ...
    const double cx = *canglezoom * x;
    const double cy = *canglezoom * y;
    const double sx = *sanglezoom * x;
    const double sy = *sanglezoom * y;

    const int dstwidthhalf = MAX(
        (int)
        ceil(MAX(MAX(MAX(fabs(cx + sy), fabs(cx - sy)), fabs(-cx + sy)), fabs(-cx - sy))),
        1
    );
    const int dstheighthalf = MAX(
        (int)
...
*/
SDL_Surface *rotozoomSurface(SDL_Surface *src, const double angle, const double zoom, const int smooth) {
...
\return The new rotozoomed surface.
*/
SDL_Surface *rotozoomSurfaceXY(SDL_Surface *src, const double angle, double zoomx, double zoomy, const int smooth) {
    SDL_Surface *rz_src;
    SDL_Surface *rz_dst;
    ...
    */
    int is32bit = src->format->BitsPerPixel == 32;
    ...
    */
    const int flipx = zoomx < 0.0;
    if (flipx)
        zoomx = -zoomx;
    const int flipy = zoomy < 0.0;
    if (flipy)
        zoomy = -zoomy;
    ...
    if (zoomy < VALUE_LIMIT)
        zoomy = VALUE_LIMIT;
    const double zoominv = 65536.0 / (zoomx * zoomx);
    ...
    */
    if (fabs(angle) > VALUE_LIMIT) {
        ...
        */
        const double sanglezoominv = sanglezoom * zoominv;
        const double canglezoominv = canglezoom * zoominv;

        /* Calculate half size */
        const int dstwidthhalf = dstwidth / 2;
        const int dstheighthalf = dstheight / 2;
...
*/
void zoomSurfaceSize(const int width, const int height, double zoomx, double zoomy, int *dstwidth, int *dstheight) {
    /*
...
*/
SDL_Surface *zoomSurface(SDL_Surface *src, double zoomx, double zoomy, const int smooth) {
    SDL_Surface *rz_src;
    int dstwidth, dstheight;
    int src_converted;
...
*/
/*@null@*/
SDL_Surface *shrinkSurface(SDL_Surface *src, const int factorx, const int factory) {
    // int result;
    SDL_Surface *rz_src;
    SDL_Surface *rz_dst = NULL;
    int src_converted = 0;
    ...
    */
    ...
    /* Get size for target */
    int dstwidth = rz_src->w / factorx;
    while (dstwidth * factorx > rz_src->w) {
        dstwidth--;
    }
    int dstheight = rz_src->h / factory;
    ...
    */
    if (is32bit == 1) {
        ...
        */
        ...
        // UNREACHABLE
        // result = shrinkSurfaceRGBA(rz_src, rz_dst, factorx, factory);
        ...
        */
        ...
        // UNREACHABLE
        // result = shrinkSurfaceY(rz_src, rz_dst, factorx, factory);
        // if (result != 0) {
        //     haveError = 1;
        //     goto exitShrinkSurface;
        // }
