...
*/
int SDL_setFramerate(FPSmanager *manager, const Uint32 rate) {
...
*/
int SDL_getFramerate(const FPSmanager *manager) {
    if (manager == NULL) {
        return -1;
    }
...
*/
int SDL_getFramecount(const FPSmanager *manager) {
    if (manager == NULL) {
        return -1;
    }
...
Uint32 SDL_framerateDelay(FPSmanager *manager) {
    ...
    * Get/calc ticks
    */
    const Uint32 current_ticks = getTicks();
    const Uint32 time_passed = current_ticks - manager->lastticks;
    manager->lastticks = current_ticks;
    const Uint32 target_ticks = manager->baseticks + (Uint32)((float)manager->framecount * manager->rateticks);

    if (current_ticks <= target_ticks) {
        const Uint32 the_delay = target_ticks - current_ticks;
        SDL_Delay(the_delay);
    } else {
        manager->framecount = 0;
        manager->baseticks = getTicks();
